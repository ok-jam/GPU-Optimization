...
중간평가 이전까지 GPU-Z를 활용하여 실시간으로 얻어지는 데이터를 cxv파일로 넘겨받아 파이썬의 dash를 통해 기본적인 웹 구현성공
앞으로는 실질적인 그래픽카드(Nvida)의 NVML을 이용하여 웹 구현예정

**중간평가 피드백**

 Dash를 이용한 웹구현을 우선시 하기 보다는 레퍼런스에서 그래픽카드에 우선적으로 적용할 수 있는 알고리즘 (그래픽 카드의 성능을 낮추지만, 에너지 소비량을 줄일 수 있는 최적구간)
